...
Shared-skill content is generated by `just update-skills`. All all-agent
`skills add` sources use copy mode so Claude's linked skill root cannot produce
self-referential leaf links. OpenCode wiring removes only links that point to
this repository's `.agents/skills` tree, preserves tool-managed links and
existing destinations, and creates only missing links. Generated skill content
and Claude/Pi registries remain gitignored.
...
is deliberate: `~/.claude/skills` is a Home Manager symlink to the generated
